...
: S_Object_GL(sp), pa_(sp->getPolyhedronAlgorithm()), displayList_(-1)
{
  createDispList();
}
...
void Polyhedron_algorithms_GL::createDispList()
{
  ...
  if(vertexes.size() > 0)
  {
    ...
    for(unsigned i = 0; i < triangles.size(); i++)
    {
      ...
      glVertex3d(vertexes[triangles[i].a]->getCordinates()[0], vertexes[triangles[i].a]->getCordinates()[1],
                 vertexes[triangles[i].a]->getCordinates()[2]);

      glVertex3d(vertexes[triangles[i].b]->getCordinates()[0], vertexes[triangles[i].b]->getCordinates()[1],
                 vertexes[triangles[i].b]->getCordinates()[2]);

      glVertex3d(vertexes[triangles[i].c]->getCordinates()[0], vertexes[triangles[i].c]->getCordinates()[1],
